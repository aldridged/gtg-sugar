...
class ProjectViewEdit extends ViewEdit{
  ...
  function display(){
    ...
    if (empty($this->ev->focus->id) && ($_REQUEST['relate_to'] == 'projects_accounts')) {
      ...
      if(isset($parent->specialbilling_c)) { $this->ev->fieldDefs['specialbilling_c']['value'] = $parent->specialbilling_c; };
      if(isset($parent->name)) { $this->ev->fieldDefs['customer_c']['value'] = $parent->name; };
    }
